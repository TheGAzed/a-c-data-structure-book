...
All the element are in an array, thus iterating over them is as simple as using an empty for loop one-liner. The \texttt{handle} function pointer, together with its \texttt{arguments}, allows direct access to the element, making manipulation is possible. If \texttt{handle} returns false, the iteration automatically stops, kinda like the \texttt{break} keyword in a classic for loop.
