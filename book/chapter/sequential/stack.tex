...
For a more in-depth explanation of lists you can infer to \autoref{pt:two} \nameref{pt:two}, but in short a linked list is a linear data structure which allows us to store data sequentially as well as store a reference to the next data item, like in \autoref{fig:linked-list}.
...
Creating a copy can be done with the \texttt{copy\_istack} function. This functionality is similar to the \texttt{memcpy} function in \texttt{<string.h>}, which takes a destination address, source address, and the size of the memory area to copy.
...
\subsection{Creating and destroying/clearing a stack of integers}
