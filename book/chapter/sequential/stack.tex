...
\subsection{Shrinking, expanding, resizing}

Infinite expansion and shrinking require the understanding of reallocating memory and implementation of the \texttt{\_istack\_resize} function. It just calls standard \texttt{realloc} when high- and low-end capacity is reached while preserving minimum chunk length.

\begin{lstlisting}[language=C, style=CodeEditor, label=lst:create-stack, title=source/sequence/istack.c]
...
\begin{lstlisting}[language=C, style=CodeEditor, label=lst:make-finite-stack, title=source/sequence/istack.c]
/// @brief Creates an empty structure.
/// @param size Size of a single element.
...
Next, \texttt{destroy\_istack} destroys the stack and all its elements while at the same time making it invalid for future usage. The parameters include a pointer to the stack and a function pointer taking in each element, allowing the user to manipulate elements prior to their destruction. The \texttt{destroy()} function pointer can primarily be used to free memory, like strings, from the heap.
...
Creating a copy can be done with the \texttt{copy\_istack} function. This functionality is similar to the \texttt{memcpy} function in \texttt{<string.h>}, which takes a destination address, source address, and the size of the memory area to copy, see \ref{lst:memcpy}.
...
Since we can't remove elements from an empty stack, the function available to aid with this issue is the \texttt{is\_empty\_istack}.
...
Each element can be accessed and manipulated through the special \texttt{each\_istack} function which iterates beginning not at the top, but at the bottom, and them making its way up to the top. The reason it starts at the bottom is because it makes printing the values easier.
...
All the element are in an array, thus iterating over them is as simple as using an empty for loop. The \texttt{handle} function pointer, together with \texttt{arguments}, allows direct access to the element, thus manipulation is possible. If \texttt{handle} returns false, the iteration automatically stops, kinda like the \texttt{break} keyword in a classic for loop.
...
\subsection{Creating and clearing/destroying a stack of integers}
...
\subsection{Pushing, peeping and popping integer elements}
